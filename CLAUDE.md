...
- **予測ターゲット**: 翌週の騰落率（回帰）
- **特徴量（Phase 1 MVP）**: テクニカル125項目 + ファンダメンタル20項目 + セクター指数34項目 + 信用倍率1項目 = 180項目
...
## 現在の状態（2026-08-01 - フェーズ6-8完了：セクター指数取得・騰落率計算完了）
...
#### ✅ 6-8. セクター指数取得・騰落率計算（完了）

**目的**: Phase 1の特徴量（34項目）として機械学習に使用

**成果物**:
- ✅ **DBテーブル作成**: `sector_indices_daily`（Alembic管理）
  - **主キー**: `id` (UUID, auto-generated)
  - **自然キー（UNIQUE制約）**: `(index_code, date)`
  - カラム: 四本値（open/high/low/close）+ 騰落率4項目
  - 騰落率: change_rate_1d/5d/20d/60d（営業日ベース、%形式）
  - Alembicマイグレーション実行完了: `20260801_1500`

- ✅ **DDD構造でデータ取得・計算機能実装**
  - Infrastructure層: `JQuantsIndexRepository`（API呼び出し）
  - Infrastructure層: `SectorIndexDailyRepository`（DB保存、UPSERT）
  - UseCase層: `FetchSectorIndicesUseCase`（日単位分割、差分取得）
  - UseCase層: `CalculateSectorIndexChangesUseCase`（騰落率計算、全量・差分）
  - Jobs層: `fetch_sector_indices.py`（全量取得バッチ）
  - Jobs層: `fetch_daily_sector_indices.py`（差分取得バッチ）
  - Jobs層: `calculate_sector_index_changes.py`（騰落率全量計算）
  - Jobs層: `calculate_daily_sector_index_changes.py`（騰落率差分計算）

- ✅ **日次ワークフロー統合**
  - `jobs/workflows/daily_data_update.py` 更新（6ステップ完成）
  - 株価取得 → 財務取得 → テクニカル指標計算 → ファンダメンタル指標計算 → **セクター指数取得** → **騰落率計算**

**実装の特徴**:
- ✅ **日単位分割**: 47指数を1日ずつ取得（レート制限対策）
- ✅ **wait=1秒**: 1日ごとに1秒待機（60件/分を確実に回避）
- ✅ **差分取得**: 最新日+1から取得（冪等性担保）
- ✅ **営業日ベース騰落率**: 60営業日前が休日・連休でも正しく計算
- ✅ **max_search_days最適化**: 1d=28, 5d=28, 20d=80, 60d=300日（2倍の安全マージン）
- ✅ **UPSERT制御**: 生データ（OHLC）のみ更新、計算済み騰落率は保持
- ✅ **型変換**: numpy型→Python標準型に変換（PostgreSQL互換）

**動作確認**:
- ✅ 全量取得成功: **108,888件**（47指数 × 約2,317日 × 10年分）
- ✅ 騰落率全量計算成功: 4項目すべて正常に%形式で保存
- ✅ 計算カバー率: 2025年以降100%達成（全期間97.41%、初期データ除く）
- ✅ 差分取得・計算成功: 日次ワークフロー6ステップすべて正常動作
...
# 日次ワークフロー（株価→財務→テクニカル→ファンダメンタル→セクター指数→騰落率）
uv run python backend/jobs/workflows/daily_data_update.py
```

**データソース**: `/v2/indices/bars/daily`（指数四本値API）

**取得指数**: 47指数（Phase 1では34指数使用、全47指数をDB保存）
- **TOPIX関連（9指数）**: TOPIX, Core30, Large70, 100, Mid400, 500, Small, 1000, Small500
- **市場別（4指数）**: プライム市場、スタンダード市場、グロース市場、グロース市場250
- **東証33業種（33指数）**: 水産・農林業〜サービス業（0040-0060）
- **その他（1指数）**: REIT

**Phase 1特徴量**: 34指数（TOPIX + 東証33業種）
- データベースの `sectors` テーブル（東証33業種マスタ）と整合
- `stock_master.sector_code` は東証33業種コード（0040-0060）を参照

**設計方針**:
- 東証33業種を採用（TOPIX-17は不採用）
- データベースの `sectors` テーブル（東証33業種）との整合性確保
- 全47指数をDB保存（将来的な分析・Phase 2以降で活用）

**レート制限**: 60件/分（株価・財務APIとは独立）

**対象期間**: 2009-02-02 〜 現在（Standardプランの取得可能期間）

**データ量**: **108,888件**（47指数 × 約2,317日）
...
✅ 完了:
  ...
  9. フェーズ6-8: セクター指数取得・騰落率計算完了（47指数、108,888件） 🎉
     - J-Quants API /v2/indices/bars/daily から全47指数取得完了
     - TOPIX 9 + 市場別 4 + 東証33業種 33 + REIT 1 = 47指数
     - Phase 1特徴量: 34指数（TOPIX + 東証33業種）使用
     - 設計方針: 東証33業種を採用（データベースsectorsテーブルと整合）
     - 騰落率計算完了（1d/5d/20d/60d、営業日ベース）
     - max_search_days最適化（28/28/80/300日、2倍安全マージン）
     - 計算カバー率: 2025年以降100%達成
     - DBテーブル `sector_indices_daily` 作成（Alembic管理）
