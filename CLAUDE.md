...
1. **Point-in-Time問題**
   - 決算発表日（`disc_date`）と株価取引日のタイムラグ
   - 未来情報混入を防ぐため、各営業日時点で「既に開示されていた最新の財務データ」を使う必要がある
   - 例: 2024年3月期決算が2024年5月10日発表 → 5月10日以降の株価データに結合

2. **訂正開示・予想修正の扱い**
   - 訂正開示があった場合の過去データの再計算ルール
   - 案A: 過去に遡って再計算（データ整合性優先）
   - 案B: 訂正開示日以降のみ新しい値を使用（実運用に近い）
   - 案C: 訂正前データをフラグ付きで保持（両方検証可能）

3. **引け後開示の扱い**
   - 同日の引け後（15:30以降）開示は翌営業日から反映すべきか？
   - `disc_time`を活用した処理分岐

4. **更新頻度と再計算範囲**
   - 日次で財務データを取得しているが、どの期間の特徴量を再計算すべきか？
   - 新規開示時、訂正開示時、予想修正時の処理を明確化

**詳細**: `docs/ml/feature-engineering.md` の「⚠️ TODO: ファンダメンタル指標のPoint-in-Time設計」を参照
...
🎯 次のセッション（Phase 1準備の残り）:
  8. フェーズ6-7: ファンダメンタル指標計算バッチ
     ...
     - 全銘柄×営業日分を計算
  9. フェーズ6-8: セクター指数取得
     - J-Quants API /v2/indices/bars/daily から取得
...
⏭️ Phase 2（精度向上 - オプション）:
 12. フェーズ7-2: 海外データ追加（Phase 1で目標未達の場合のみ）
     - 為替、米国株指数、商品等 +10項目
     - センチメント指標追加（空売り比率等）
...
⏭️ 運用開始:
 13. フェーズ7-3: 週次推論パイプライン実装
 14. フェーズ6-10: GCPデプロイ（自動化）
```
